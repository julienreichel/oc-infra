...
DEV_SERVICE_FILE="k8s/oc-provider-backend-dev-svc.yaml"
...
# 2️⃣ Apply ExternalName service to reach localhost:3001
...
kubectl -n "$NS" patch ingress "$INGRESS_NAME" --type='json' \
  -p='[
    {"op":"replace","path":"/spec/rules/0/http/paths/1/backend/service/name","value":"oc-provider-backend-dev"},
    {"op":"replace","path":"/spec/rules/0/http/paths/1/backend/service/port/number","value":3001}
  ]'
...
echo "Done! Your local backend on http://localhost:3001 now handles /api for https://provider.localhost"
